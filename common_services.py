...
class commonServices:
    ...
    def userAgentsList(self):
        ...
        return self.user_agents_list

    @staticmethod
    def checkIfItemIsListType(data, index):
        try:
    ...
    def findFirstAndLastName(self, personName):
        try:
            name = personName.strip().split(' ')
            firstName = ''
            lastName = ''
            if len(name) >= 2:
    ...
    def findPaginationNumber(self, url, typeMovie):
        try:
            if typeMovie == 'web-series':
                url = url.replace('-movies', '')
            rep = requests.get(url)
            pageData = html.fromstring(rep.text)
            return commonServices.checkIfItemIsListType(
                pageData.xpath('//div[contains(@class,"_flx _mvlstw ")]/@data-totalcount'), 0)
        except Exception as error:
            print(error, 'error')

    def checkResultData(self, resultData):
        if resultData is None:
            exit()
        else:
            return resultData

    def convertTwoListIntoDict(self, list1, list2):
        try:
            people = {}
            for i in range(len(list2)):
                people[list1[i]] = list2[i].replace('/name/', '')
            return people
        except Exception as error:
            print(error)

    def getMovieAndYear(self, movieNameWithYear):
        try:
            movieNameWithYear = movieNameWithYear.split('(')
            movieName = movieNameWithYear[0].strip()
            movieYear = (movieNameWithYear[1].replace(')', '')).strip()
            return movieName, movieYear
        except Exception as error:
            print(error)

    def getDigitPages(self, url, headers):
        try:
            response = requests.get(url, headers=headers)
            htmlData = response.text
            data = html.fromstring(htmlData)
            results = commonServices.checkIfItemIsListType(data.xpath('//div[@class="result_heading"]/text()'), 0)
            pages = commonServices.checkIfItemIsListType(results.split('RESULTS FOUND'), 0).strip()
            if pages:
                pages = math.ceil(int(pages) / 21)
            return pages
        except Exception as error:
            print(error)

    def getPrimePages(self, url, headers):
        try:
            response = requests.get(url, headers=headers)
            htmlData = response.text
            data = html.fromstring(htmlData)
            data = data.xpath('//script[@type="text/template"]//text()')
            pages = None
            for i in data:
                try:
                    pages = math.ceil(int(json.loads(i)['props']['opts']['totalItems']) / 20)
                except Exception as error:
                    print(error)
            if pages:
                return pages
